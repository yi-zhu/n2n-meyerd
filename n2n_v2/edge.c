...
static void readFromMgmtSocket(n2n_edge_t *eee, int *keep_running) {
    ...
    if (recvlen >= 5) {
        ...
        if (0 == memcmp(udp_buf, "peers", 5)) {
            ...
            c = 0;
            for (lpi = sglib_hashed_peer_info_t_it_init(&it, eee->known_peers);
                 lpi != NULL; lpi = sglib_hashed_peer_info_t_it_next(&it)) {
                c++;
                msg_len = 0;
                msg_len += snprintf((char *) (udp_buf + msg_len), (N2N_PKT_BUF_SIZE - msg_len),
            ...
            c = 0;
            for (lpi = sglib_hashed_peer_info_t_it_init(&it, eee->pending_peers);
                 lpi != NULL; lpi = sglib_hashed_peer_info_t_it_next(&it)) {
                c++;
                msg_len = 0;
...
static void supernode2addr(n2n_sock_t *sn, const n2n_sn_name_t addrIn) {
    ...
    if (supernode_host) {
        ...
        printf("%s - [系统]当前域名：%s,当前IP：%hhu.%hhu.%hhu.%hhu \n",theDate, supernode_host,sn->addr.v4[0],sn->addr.v4[1],sn->addr.v4[2],sn->addr.v4[3]);
...
#endif
//    system("chcp 65001"); //强制使用编码，解决汉字不显示
    for (i = 1; i < argc; ++i) {
        printf("[系统]: 参数 %s \n",argv[i]);
        if (argv[i][0] == '@') {
        ...
        switch (opt) {
            case 'K': {
                if (encrypt_key) {
                    ...
                    eee.keyschedule[N2N_PATHNAME_MAXLEN -
                                    1] = 0; /* strncpy does not add NULL if the source has no NULL. */
